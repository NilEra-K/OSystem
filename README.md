# OSystem
A personal project to learning Unix-C on Linux(Ubuntu 22.04 LTS)
...
***TCP协议保证数据传输的可靠性*** <p>
***超时重传 & 反向确认*** <p>
**反向确认 :** 当发出信息后, 接收方受到信息后需要反向发送一条确认信息, 即反向确认 <p>
**超时重传 :** 当发出信息后, 一段时间内没有受到回应, 发送方会再一次发送信息, 即超时重传 <p>
TCP的协议栈底层在向另一端发送数据时, 会要求对方在一个给定的时间窗口内返回**确认**, 如果超过了这个时间窗口仍然没有收到确认, 则 TCP 会**重传**数据并等待更长的时间 <p>
只有在数次重传均告失败之后, TCP 才会最终放弃, TCP含有用于动态估算数据往返时间 ***(Round-Trip Time, RTT)*** 的算法, 因此它知道等待一个确认需要多长时间

***TCP协议保证数据传输的有序性*** <p>
TCP的协议栈底层在向另一端发送数据的时候, 会为所发送的每个字节指定一个序列号, 即使这些字节没有能够按照发送时的顺序到达接收方, 接收方的 TCP也可以根据他们的序列号重新排序, 再把最后的结果交给应用程序

***TCP协议是全双工的*** <p>
在给定的连接上, 应用程序在任何时候都既可以发送数据, 也可以接收数据, 因此, TCP必须跟踪每个方向上数据流的状态信息, 如序列号和通告窗口的大小

**[笔试面试基本必考] TCP 建立连接的过程** <p>
***三路握手***
- 客户机的 TCP协议栈向服务器发送一个 SYN分节, 告知对方自己将在连接中发送数据的初始序列号, 谓之主动打开
- 服务器的 TCP协议栈向客户机发送一个单个分节, 其中不仅包括对客户机 SYN分节的 ACK应答, 还包括服务器自己的 SYN分节, 以告知对方自己在同一个连接中发送数据的初始序列号
- 客户机的 TCP协议栈向服务器返回 ACK应答, 以表示对服务器所发 SYN的确认

***用更为通俗的语言描述一下就是 :*** <p>
- 首先客户机会将向服务器发送一个 SYN分节, 即请求与服务器建立连接, 初始序列号或者说 32位序号, 也就是下图中 _`SYN[J]`_ 中的 **[ J ]**
- 服务器接收到客户机发送的 SYN分节后, 会发送一个 ACK应答, 也就是下图中 _`ACK[J+1]`_ 中的 **[ J+1 ]**, **[ J+1 ]** 表示是在客户机发送的 **[ J ]** 上的加一个字节, 表示目标服务器确实收到了该分节, 还会发送一个服务器自己的 SYN分节, 告知客户机自己的序列号, 即对应下图中的 _`SYN[K]`_
- 同理, 客户机为了表示自己是服务器的目标, 会作出应答, 向服务器发送 _`ACK[K+1]`_ , 表示自己确实受到了服务器发送的序列号, 与服务器建立连接

一些名词的解释: 
- **SYN(Synchronization) 同步**
- **ACK(Acknowledge) 确认**
- 关于 **SYN** 和 **ACK** 可以在网络基础中的 TCP首部中找到, 它是一个比特位, 通过置 0或者 置 1来表示有无

![avatar](./07_dir_net/src/three-wayHandshake.png)

**[补充]** 有一种黑客攻击的方式叫做 DDOS(也叫做分布式拒绝服务攻击), 是一种非常粗暴的攻击方式, 其正是利用了 TCP三路握手的漏洞来实现的

**TCP 交换数据的过程** <p>
![avatar](./07_dir_net/src/exchangeData.png)
简单来说就是: 
- 客户机给服务器发送数据包, 服务器给客户机回复 ACK
- 服务器给客户机发送数据包, 客户机个服务器回复 ACK

**[笔试面试基本必考] TCP 终止连接的过程** <p>
- 客户机或者服务器主动关闭连接, TCP协议栈向对方发送 FIN分节, 表示数据通信结束, 如果此时上有数据滞留在发送缓冲区中, 则 FIN分节跟在所有为发送的数据之后
- 接收到 FIN分节的另一端执行被动关闭, 一方面通过 TCP协议栈向对方发送 ACK应答, 另一方面向应用程序传递文件结束符
- 一段时间以后, 方才接收到 FIN分节的进程关闭自己的连接, 同时通过 TCP协议栈向读方发送 FIN分节
- 对方在收到 FIN分节后发送 ACK应答
- 这个过程也叫做 **TCP的四次挥手**

![avatar](./07_dir_net/src/FourWaves.png)

**相关结构体** <p>
套接字接口库通过地址结构定位一个通信主体, 可以是一个文件, 可以是一台远程主机, 也可以是执行者自己 <p>
_**`struct sockaddr{...};`**_ <p>
...
_**`struct in_addr{...};`**_ <p>
- 网络地址结构, 用于 `AF_INET` 域的IPv4网络通信
  ```
  struct in_addr{
    in_addr_t s_addr;
  };

  typedef uint16_t inport_t;  // 无符号 16位整数
  typedef uint32_t inport_t;  // 无符号 32位整数
  
  ```

_**`struct sockaddr_in{...};`**_ <p>
- 网络地址结构, 用于 `AF_INET` 域的IPv4网络通信
  ```
  struct sockaddr_in{
    sa_family_t sin_family; // 地址族
                            // (AF_INET)
    in_port_t sin_port;     // 端口号(0~65535)
                            // unsigned short
...
_**`int bind(int sockfd, struct sockaddr const* addr, socklen_t)`**_
- 使用时需引用 `#include <sys/socket.h>` 头文件
- 功能 : 将套接字和本地的地址结构绑定在一起
- 参数 : <p>
  **sockfd :** 套接字描述符 <p>
  **addr :** 自己的地址结构, 是 sockaddr型的变量, sockaddr没有实际意义, 是一个泛式化的参数, 使用时需要传入自己的地址结构, 因为存在不同的地址结构, 所以需要强制类型转换, 转化为 sockaddr类型的指针, 从而解决了需要传入不同类型的地址的问题, 也就是说不管是 `sockaddr_in`类型的结构体还是 `sockaddr_un`类型的结构体, 都只需要强制转化为 `sockaddr`类型即可 <p>
  **addrlen :** 地址结构字节数 <p>
  **返回值 :** 成功返回 0, 失败返回 -1

_**`int listen(int sockfd, int backlog)`**_
- 使用时需引用 `#include <sys/socket.h>` 头文件
- 功能 : 启动侦听, 在指定的套接字上启动对连接请求的侦听功能
- 参数 : <p>
  **sockfd :** 套接字描述符, 在调用此函数之前是一个主动套接字, 是不能感知连接请求的, 在调用此函数并成功返回之后, 是一个被动套接字, 具有感知连接请求的的能力 <p>
  **backlog :** 未决连接请求队列的最大长度, 一般取不小于 1024的值 <p>
  **返回值 :** 成功返回 0, 失败返回 -1

_**`int accept(int sockfd, struct sockaddr* addr, socklen_t* addrlen)`**_
- 使用时需引用 `#include <sys/socket.h>` 头文件
- 功能 : 等待并接受**连接请求**, 在指定套接字上阻塞, 直到连接建立完成
- 参数 : <p>
  **sockfd :** 监听套接字的描述符 <p>
  **addr :** 输出连接请求发起方的地址信息 (该参数为输出型参数) <p>
  **addrlen :** 输出连接请求发起方的地址信息字节数(该参数为输出型参数) <p>
...
_**`ssize_t send(int sockfd, void const* buf, size_t count, int flags)`**_
- 使用时需引用 `#include <sys/socket.h>` 头文件
- 功能 : 发送数据
- 参数 : <p>
  **sockfd :** 套接字的描述符 <p>
  **buf :** 要发送的字节缓冲区 <p>
  **flags :** 若 flags取 0 则与 write函数完全等价, 另外也可以取以下值—— <p>
  `MSG_DONTWAIT` 以非阻塞方式接收数据 <p>
  `MSG_OOB` 接收带外数据 <p>
...
**[补充] 关于 IP地址到底怎么传 :** 服务器代码中的 IP地址应该传入主机的 IP地址, 客户端的代码要传入服务器端主机的 IP地址, 这里目前还无法跨网段连接

**通信终止** <p>
***客户机主动终止通信过程***
- 在某个特定时刻, 客户机认为已经不再需要服务器需要继续为其提供服务器了, 于是他在接收完最后一个响应包后, 通过 `close` 函数关闭与服务器通信的套接字, 客户机的TCP协议栈向服务器发送 `FIN` 分节并得到对方的 *ACK* 应答, 服务器专门负责与该客户机通信的子进程, 此刻正试图通过 `recv` 函数接收下一个请求包, 结果却因为收到来自客户机的 *FIN* 分节而返回 `0` , 于是该子进程退出收发循环, 同时通过 `close` 函数关闭套接字, 导致服务器的 TCP协议栈向客户机发送 *FIN* 分节, 使对方进入 *TIME_WAIT* 状态, 并在收到对方的 *ACK* 应答之后, 自己进入 *CLOSED* 状态, 随着收发进程的退出, 服务器的子进程终止, 并在服务器主进程的`SIGCHLD(17)` 信号处理函数中被回收, 通信过程宣告结束

***服务器主动终止通信过程*** 
- 服务器专门负责和某个特定客户机通信的子进程, 在运行是出现错误, 不得不调用 `close` 函数关闭连接套接字, 或者直接退出, 甚至被信号杀死, 于是服务器的 TCP协议栈向客户机发送 *FIN* 分节并得到对方的 *ACK* 应答
- **A.** 如果客户机此时正试图通过 `recv` 函数接收响应包, 那么该函数会返回 0, 客户机可据此判断服务器已经宕机, 直接通过 `close` 函数关闭与服务器通信的套接字, 终止通信过程
...
- 请求头部由关键字/值组成, 每行一对, 关键字和值用英文冒号 ":" 分隔, 请求头部通知服务器有关于客户端请求的信息, 典型的请求头有:
  - **`Accept:`** 告诉服务器自己接受什么类型的介质, `*/*` 表示任何类型, `type/*` 表示该类型下的所有子类型, 例如: `text/html` , `image/pneg`
  - **`Host:`** 客户端指定自己想访问的 Web服务器域名
  - **`User_Agent:`** 浏览器表明自己的身份, 是那种浏览器
  - **`Referer:`** 浏览器 Web服务器表明自己是从哪个网页 URL获得点击但年年嗯前请求中的网址
  - **`Connection:`** 表示是否需要持久连接, 有 `close` 或 `keep-alive` 字段, `close` 表示之进行一次数据的收发, `keep-alive` 表示可以进行多次的数据收发

***HTTP 响应***

![avatar](./07_dir_net/src/back.png)

- HTTP 响应也由三个部分组成, 分别是: **状态行, 响应头, 空行, 响应正文**
- 当浏览者访问一个网页时, 浏览者的浏览器会向网页所在的服务器发送请求, 当浏览器接收并显示网页前, 此网页所在的服务器会返回一个包含 HTTP状态码的信息头(Server Header)用以响应浏览器的请求, 常见的状态码有: 
  - `200 OK` : 客户端请求成功
  - `400 Bad Request` : 客户端请求有语法错误, 不能被服务器所理解
  - `403 Forbidden` : 服务器收到请求, 但是决绝提供服务
...
***URL: 统一资源定位器, 也就是我们所说的链接, 一个完整的 URL 包括域名、端口号、文件内容三部分*** <p>
例如: **https://mbd.baidu.com:80/newspage/data/landingsupe**
- mbd.baidu.com 域名
- 80 端口号
- /newspage/data/landingsupe

---
### ✨ 线程管理
**线程的基本概念** <p>
- 线程就是程序的执行路线, 即进程内的控制序列, 或者说进程的子任务
- 一个进程可以同时拥有多个线程, 即同时被系统调度的多条执行路线, 但至少要有一个主线程
- 一个进程的所有线程都共享进程的代码区, 数据区, BSS区, 堆区, 命令行参数和环境变量, ***唯有栈区是一个线程一个***
- 一个进程的所有线程共享内核中与进程有关的部分资源, 如文件描述符表、信号处理函数、工作目录、各种 ID等
- 一个进程的每个线程都有一个独立的 ID、独立的寄存器上下文、独立的调度策略和优先级、独立的信号掩码、独立的 `errno` 以及线程私有数据
- 可以说: 进程是线程的容器
...
**线程调度**
- 系统内核中专门负责线程调度的处理单元被称为**调度器**, 调度器将所有处于就绪状态(没有阻塞于任何系统调用上)的线程排成一个队列, 即所谓**就绪队列**
- 调度器从就绪队列中获取队首线程, 为其分配一个时间片, 并令处理机执行该线程, 一段时间后——
  - 该线程的时间片耗尽, 调度器立即终止该线程, 并将其排到就绪队列的未断, 接着从队首获取下一个线程
  - 该线程时间片未耗尽, 但需要阻塞于某系统调用, 比如等待 I/O或者睡眠, 调度器会终止该线程, 将其从就绪队列移至等待队列(也叫 *阻塞队列* ), 直到其等待的条件满足后, 再被移回就绪队列
- ***在低优先级线程执行期间, 有高优先级线程就绪, 后者会抢占前者的时间片***
- 若就绪队列为空, 则系统内核进入空闲状态, 直至其为非空

**时间片** <p>
- ***调度器分配个每个线程的时间片长短, 对系统的行为和性能影响很大***, 如果时间片过长, 线程必须等待很长时间才能重新获得处理机, 这就降低了整个系统的并行性, 用户会感觉到明显的响应延迟, 如果时间片过短, 大量的时间会浪费在线程切换上, 同时降低了虚拟内存的存储命中率(这个在计算机组成原理课程中有涉及), 线程的时间局部性无法得到保证
- 某些 Unix 系统倾向于为线程*分配较长的时间片, 希望通过扩大系统吞吐率来改善其整体表现* ; 而另一些 Unix 系统则更倾向于*为线程分配较短的时间片, 以提升系统的交互性* <p>
*Linux 系统根据线程在不同时间的具体表现, 为其动态分配时间片, 在吞吐率和交互性之间寻求最佳平衡点*

**线程的基本特点**
- 线程是进程的一个实体, 可作为系统独立调度和分派的基本单位
- 线程有不同的状态, 系统提供了多种线程控制原语, 如创建、终止、取消等
- 线程可以使用的大部分资源都是隶属于进程的, 即使是在特定线程中动态分配的资源, 也同样为进程所拥有
- 一个进程中可以有多个线程 *并发* 地运行, 他们可以执行相同的代码, 也可以执行不同的代码
- 同一个进程的多个线程都在同一个地址空间内活动, 相较于进程, 线程的系统开销小, 任务切换块
- 进程空间内的代码和数据对于该进程的每个线程而言都是共享的, 因此同一个进程的不同线程之间不存在通信问题, 自然也就不需要类似与 IPC的通信机制
- 线程之间虽然不存在通信问题, 但是存在冲突问题, 同样是因为数据共享, 当一个进程的多个线程 **"同时"** 访问一份数据时, 线程间的冲突可能造成逻辑甚至系统错误

**POSIX 线程** <p>
POSIX标准(前面的内容)提供的接口

**相关函数** <p>
_**`int pthread_create(pthread_t* tid, pthread_attr_t const* attr, void* (*start_routine)(void*), void* arg)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 创建新线程
- 参数 : <p>
  **tid :** 输出线程 ID, `pthread_t` 即为 `unsigned long int` <p>
  **attr :** 线程属性, `NULL` 表示缺省属性 <p>
  **start_routine :** 线程过程函数指针, **所指向的函数将在被创建的线程中执行** <p>
  **arg :** 传递给线程过程函数的参数 <p>
  **返回值 :** 成功返回 0, 失败返回错误码
- 例:
  ```
  void* fun(void* args){ // 线程过程函数
    ...;
  }
  pthread_t tid;
  ...
  ```
- 补充说明:
  - `pthread_create()` 函数本身并不调用线程过程函数, 而是在系统内核中开启独立的线程, 并立即返回, 在该线程中执行线程过程函数
  - `pthread_create()` 函数返回时, 所指定的线程过程函数可能尚未执行, 也可能正在执行, 甚至可能执行完毕
  - `pthread_create()` 函数创建的多个子进程, 在时间上"同时"运行, 如不附加任何同步条件, 则他们每一个的执行先后顺序是完全无法确定的, 这就叫做 *自由并发*

_**`void* thread_proc(void* arg){...}`**_
- 功能 : 定义线程过程函数, 该函数会在所创建的线程中被执行, 代表了线程的任务
- `main` 实际就是主线程的线程过程函数
- `main` 函数一旦返回, 主线程即告结束; 主线程一旦结束, 进程即告结束; 进程一旦结束, 其所有子进程通通结束
- **线程参数 :**
  - 传递给线程过程函数的参数是一个泛型指针 `void*` , 它可以指向任何类型的数据: 基本类型的变量、结构体类型的变量、数组类型的变量等等, 但必须保证在线程过程函数执行期间, 该指针指向的内存空间持久有效, 直到线程过程函数不再使用它为止
  - 调用 `pthread_create()` 函数的代码在用户空间, 线程过程函数的代码也在用户空间, 但创建线程的动作由系统内核完成, 因此传递给线程过程函数的参数也不得不经由系统内核传递给线程过程参数, `pthread_create()` 函数的过程参数 `arg` 参数负责将线程过程参数带入系统内核

**汇合线程** <p>
在父线程中调用 `pthread_join()` 函数等待子线程的终止, 并回收其资源, 如果调用此函数时子线程已经终止, 该函数会立即返回; 如果调用该函数时子线程尚在运行中, 该函数会阻塞, 直至子线程终止, 该函数再返回成功的同时通过其 `retval` 参数向调用者输出子线程线程过程函数的返回值
![avatar](./08_dir_thread/src/threadJoin.png)

**相关函数** <p>
_**`int pthread_join(pthread_t tid, void** retval)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 等待子线程终止, 即其线程过程函数返回, 并与之汇合, 同时回收该线程的资源 
- 参数 : <p>
  **tid :** 线程 ID
  **retval :** 输出线程过程函数中的返回值
  **返回值 :** 成功返回 0, 失败返回错误码

**分离线程** <p>
一个线程在默认情况下都是可汇合线程, 这样的线程在终止以后其资源会被保留, 其中含有线程过程函数的返回值及线程 ID等信息, 父线程可以通过 `pthread_join()` 函数获得线程过程函数的返回值, 同时释放这些资源 <p>
如果在创建线程之后, 对其调用 `pthread_detach()` 函数并返回成功, 该线程即成为分离线程, 这样的线程终止以后其资源会被系统自动回收, 不需要也无法通过 `pthread_join()` 函数汇合它
![avatar](./08_dir_thread/src/threadDetach.png)
...
**线程 ID** <p>
`pthread_t` 类型的线程 ID 是 POSIX 线程库内部维护的线程的唯一标识, 通常表现为一个很大的整数, 跨平台, 可移植
`syscall(SYS_gettid)` 函数返回值是一个 `long int` 类型整数, 是系统内核产生线程唯一标识, 一个进程的PID 实际上就是它的主线程的 TID

**相关函数** <p>
_**`pthread_t pthread_self(void)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 返回调用线程的 ***(POSIX线程库的)*** TID
- 参数 : <p>
  **返回值 :** 返回调用线程的 TID

_**`long int syscall(SYS_gettid)`**_
- 使用时需引用 `#include <sys/syscall.h>` 头文件
- 功能 : 返回调用线程的 ***(系统内核的)*** TID
- 参数 : <p>
  **返回值 :** 返回调用线程的 TID

_**`int pthread_equal(pthread_t t1, pthread_t t2)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 判断两个线程 ID是否相等
- 参数 : <p>
  **返回值 :** 若两个参数所标识的 TID相等返回非 0, 否则返回 0
- `pthread_t` 类型在不同的系统会被实现为不同的数据类型, 甚至可能会使用结构体, 因此判断两个线程 ID相等或不相等, 最好不要使用 **"=="** 或 **"!="** 运算符, 因为这些关系运算符只能用于 C语言内置的简单类型, 而对于结构类型的数据并不适用

**并发冲突** <p>
当多个线程同时访问其共享的进程资源时, 如果不能相互协调配合, 就难免会出现数据不一致或不完整的问题, 这种现象被称为线程间的并发访问冲突, 简称 *并发冲突*
> 假设有整形全局变量 `g_cn` 被初始化为 0 <p>
  启动两个线程, 同时执行如下线程过程函数, 分别对该全局变量做 1000000次类加, 如下代码——<p>
  两个线程结束后, `g_cn` 理想结果应该是 2000000, 但是实际情况往往小于 2000000, 且结果不尽相同
...
***理想的原子操作*** <p>
| 线程 1 指令 | 线程 1 结果 | 内存 | 线程 2 结果 | 线程 2 指令 |
| :-: | :-: | :-: | :-: | :-: |
| *指令* | *eax* | *g_cn* | *eax* | *指令* |
| `mov1 g_cn, %eax` | 0 | 0 | - | - |
| `add $ , eax` | 1 | 0 | - | - |
...
**线程同步** <p>
- 缺省状态下, 一个进程中的线程是以异步方式运行的, 即各自运行各自的, 彼此间不需要保持步调的协调一致
- 某些情况下, 需要在线程之间建立起某种停等机制, 即一或多个线程有时必须停下来, 等待另外一或多个线程执行完一个特定的步骤以后才能继续执行, 这就叫同步
- **线程同步能带来的好处——**
  - *解决并发冲突问题 :* 任何时候只允许一个线程持有共享数据, 其他线程必须阻塞于调度队列之外, 直到数据持有者不再持有该数据为止
  - *解决资源竞争问题 :* 任何时候只允许部分线程拥有有限的资源, 其他线程必须阻塞于调度队列之外, 直到资源的拥有者主动释放其所拥有的资源为止
  - *解决条件等待问题 :* 当某些条件一时无法满足时, 一些线程必须阻塞于调度队列之外, 直至令该条件满足的线程用信号唤醒他们为止
- **如何达到线程同步 :** 互斥锁、条件变量

**互斥锁** <p>
- 线程间可以通过互斥锁解决资源竞争的问题
- 任何时候都只能一个线程持有互斥锁, 即加锁成功, 在其只有该互斥锁的过程中, 其他线程对锁的加锁动作都会引发阻塞, 只有当持有互斥锁的线程主动解锁, 那些在加锁动作上阻塞的线程中的一个采用恢复运行并加锁成功
- `pthread_mutex_t` 表示互斥锁数据类型

**相关函数** <p>
_**`int pthread_mutex_init(pthread_mutex_t* mutex, pthread_mutexattr_t const* attr)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 初始化互斥体/互斥锁
- 参数 : <p>
  **mutex :** 互斥体/互斥锁 <p>
  **attr :** 互斥体/互斥锁属性 <p>
  **返回值 :** 成功返回 0, 失败返回错误码
- 当然也可以使用静态方法初始化互斥锁(更简单/常用): `pthread_mutex_t mutex = PTHREAD_MUTEX_INITALIZER`

_**`int pthread_mutex_lock(pthread_mutex_t* mutex)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 锁定互斥体/互斥锁
- 参数 : <p>
  **mutex :** 互斥体/互斥锁 <p>
...
_**`int pthread_mutex_unlock(pthread_mutex_t* mutex)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 解锁互斥体/互斥锁
- 参数 : <p>
  **mutex :** 互斥体/互斥锁 <p>
...
_**`int pthread_mutex_destory(pthread_mutex_t* mutex)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 销毁互斥体/互斥锁
- 参数 : <p>
  **mutex :** 互斥体/互斥锁 <p>
...
**条件变量** <p>
- 一个线程在某种条件不满足的情况下, 无法进行后续工作, 这时它就可以 *睡入* 某个条件变量, 这时会有其他线程为其创建条件, 一旦条件满足, 可以唤醒那些在相应条件变量中睡眠的线程继续运行
- ***条件变量就是当线程条件不满足的时候, 睡入的地方***
- 通过 `pthread_cond_t` 类型来表示条件变量

**相关函数** <p>
_**`int pthread_cond_init(pthread_cond_t* mutex, const pthread_condattr_t* attr)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 初始化条件变量
- 参数 : <p>
  **cond :** 条件变量 <p>
  **attr :** 条件变量属性 <p>
  **返回值 :** 成功返回 0, 失败返回错误码
- 当然也可以使用静态方法初始化条件变量(更简单/常用): `pthread_cond_t mutex = PTHREAD_COND_INITALIZER`


_**`int pthread_cond_wait(pthread_cond_t* cond, pthread_mutex_t* mutex)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 睡入条件变量
- 参数 : <p>
  **cond :** 条件变量 <p>
  **mutex :** 互斥锁, 这把锁帮助线程在睡入时为其他线程解锁, 然后当条件满足时再通过者把锁夺回锁的控制权 <p>
  **返回值 :** 成功返回 0, 失败返回错误码
- `pthread_cond_wait` 函数会令调用线程进入阻塞状态, 直到条件变量 `cond` 收到信号为止, 阻塞期间互斥体 `mutex` 被解锁
- 条件变量必须与互斥体配合使用, 为防止多个线程同时进入条件等待队列时发生竞争
  - 线程在调用 `pthread_cond_wait` 函数前必须先通过 `pthread_mutex_lock` 函数锁定 `mutex` 互斥体
  - 在调用线程进入条件等待队列之前, `mutex` 互斥体一直处于锁定状态, 直到调用线程进入条件等待状态才被解锁
  - 当调用线程即将从 `pthread_cond_wait` 函数返回时, `mutex` 互斥体会重新锁定, 同时调用该函数之前的状态

_**`int pthread_cond_signal(pthread_cond_t* cond)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 唤醒在条件变量中睡眠的一个线程
- 参数 : <p>
  **cond :** 条件变量 <p>
  **返回值 :** 成功返回 0, 失败返回错误码
- 一个线程调用 `pthread_cond_wait` 函数进入阻塞状态, 直到条件变量 `cond` 收到信号为止, 阻塞期间互斥锁 `mutex` 会被释放, 另一个线程通过 `pthread_cond_signal` 函数向条件变量 `cond` 发送信号, 唤醒在其中睡眠的一个线程, 该线程即从 `pthread_cond_wait` 函数中返回, 同时获得互斥锁 `mutex`

_**`int pthread_cond_destory(pthread_cond_t* cond)`**_
- 使用时需引用 `#include <pthread.h>` 头文件
- 功能 : 销毁条件变量
- 参数 : <p>
  **cond :** 条件变量 <p>
...
**生产者-消费者(Producer-Consumer)问题** <p>
亦称有界缓冲区(Bounded-Buffer)问题
- 两个线程共享一个公共大小的缓冲区, 其中一个线程作为生产者, 负责将消息放入缓冲区, 而另一个线程则作为消费者, 负责从缓冲区中提取消息
- 假设缓冲区已满, 若生产者还想放入消息, 就必须等待消费者线程从缓冲区中提取消息以产生足够的空间
